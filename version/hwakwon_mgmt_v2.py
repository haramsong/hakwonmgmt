...
class Ui_Season_Master_Mgmt(QDialog):
    ...
    def setupUi(self):
        ...
        self.nameLE.setValidator(QRegExpValidator(QRegExp("S-[0-9]{6}")))
        self.nameLE.setPlaceholderText("S000000")  # 입력 가이드 (S000000)
        self.gridLayout.addWidget(self.nameLE, 1, 1, 1, 1)

        self.fld1_LE = QLineEdit(self.gridLayoutWidget)  #내용
        self.fld1_LE.setText(str(clickedRcdInfo[3]))
        self.fld1_LE.setValidator(QRegExpValidator(QRegExp("[0-9]{2}년 [0-9]{1}학기")))
        self.fld1_LE.setPlaceholderText("00년 0학기")  # 입력 가이드 (00년 0학기)
        ...
        self.fld2_DE.setDate(QDate.fromString(clickedRcdInfo[4], 'yyyy-MM-dd')) #시작일
        ...
        self.fld3_DE.setDate(QDate.fromString(clickedRcdInfo[5], 'yyyy-MM-dd'))  #종료일
    ...
    def accept(self):
        ...
        if self.idLabel.text() == "":
            ...
            return
        AA_create_arr = [clickedRcdInfo[0], int(self.idLabel.text()[2:7]), self.nameLE.text(), self.fld1_LE.text(),
                         self.fld2_DE.text(),
        ...
        if scr_control_event == "CRT":
            ...
            df_super_list[0][10] = df_super_list[0][10] + 1
...
class Ui_Room_Master_Mgmt(QDialog):
    ...
    def addMember(self):
        if self.nameLE.text() != " " and self.fld1_LE.text() != "":
            self.idLabel.setText(clickedRcdInfo[0] + str(df_super_list[1][10]))
    ...
    def accept(self):
        ...
        BB_create_arr = [clickedRcdInfo[0], df_super_list[1][10], self.nameLE.text(), self.fld1_LE.text(), self.comboBox1.currentIndex() + 1,
        ...
        if scr_control_event == "CRT":
            ...
            df_super_list[1][10] = df_super_list[1][10] + 1
            df = pd.DataFrame(df_super_list, columns=df_super_col)
            df.to_excel('table/super_master.xlsx', index=False)
        else:
...
class Ui_Lesson_Master_Mgmt(QDialog):
        ...
        for i in range(len(tabWidget_less_schl_arr)):
            ...
            for j in range(len(tabWidget_less_schl_arr[i])):
                ...
                for j in range(len(days_text_info_arr)):
                    if days_text_info_arr[j][0] != master_ID: continue                       #강좌
                    if days_text_info_arr[j][1] != df_lesson_schedule_list[i][4]: continue   #그룹 키
                    if days_text_info_arr[j][2] != df_lesson_schedule_list[i][5]: continue   #강의실 ID
                    if days_text_info_arr[j][3] != df_lesson_schedule_list[i][6]: continue   #시간 ID
                    if days_text_info_arr[j][4] != df_lesson_schedule_list[i][7]: continue   #요일 ID
                    self.tableWidget.setItem(i, 2, item)
                    item = self.tableWidget.item(i, 2)
                    item.setText(days_text_info_arr[j][5])
            old_group_key = tabWidget_less_schl_arr[i][0]

    def addMember(self):
        if self.nameLE.text() != " " and self.fld1_LE.text() != "":
            self.idLabel.setText(clickedRcdInfo[0] + str(df_super_list[2][10]))
    ...
    def refreshInfo(self):
        self.close()
        Ui_MainWindow().init_preparetion()
        self.__init__()
        self.exec_()
    ...
    def accept(self):
        ...
        CC_create_arr = [clickedRcdInfo[0], df_super_list[2][10], self.nameLE.text(), self.fld1_LE.text(), self.comboBox1.currentIndex() + 1,
                         self.fld2_LE.text(), self.fld3_LE.text(), self.fld4_LE.text(), self.remark1LE.text(),
                         self.remark2LE.text(), "", "", "", "", "", ""]
        ...
        if scr_control_event == "CRT":
            ...
            df_super_list[2][10] = df_super_list[2][10] + 1
            df = pd.DataFrame(df_super_list, columns=df_super_col)
            df.to_excel('table/super_master.xlsx', index=False)
        else:
            for i in range(len(CC_master_list)):
...
class Ui_Teacher_Master_Mgmt(QDialog):
    ...
    def addMember(self):
        if self.nameLE.text() != " " and self.fld1_LE.text() != "":
            self.idLabel.setText(clickedRcdInfo[0] + str(df_super_list[3][10]))
    ...
    def accept(self):
        ...
        DD_create_arr = [clickedRcdInfo[0], df_super_list[3][10], self.nameLE.text(), self.fld1_LE.text(),
...
class Ui_Student_Master_Mgmt(QDialog):
    ...
    def addMember(self):
        if self.nameLE.text() != " " and self.fld1_LE.text() != "":
            self.idLabel.setText(clickedRcdInfo[0] + str(df_super_list[4][10]))
    ...
    def accept(self):
        ...
        EE_create_arr = [clickedRcdInfo[0], df_super_list[3][10], self.nameLE.text(), self.fld1_LE.text(),
                         gender, self.comboBox1.currentText(),
        ...
        if scr_control_event == "CRT":
            ...
            df_super_list[4][10] = df_super_list[4][10] + 1
            df = pd.DataFrame(df_super_list, columns=df_super_col)
            df.to_excel('table/super_master.xlsx', index=False)
        else:
...
class Ui_MainWindow(QMainWindow):
    ...
    def setupUi(self):
        ...
        for i in range(len(tab_arr)):
            ...
            # # self.comboBox_select.setCurrentIndex(season_active_idx)
            ...
            # tab_arr[i][1].setSelectionMode(QAbstractItemView.NoSelection)
    ...
    def handleCellClicked(self, it, col):
        ...
        for i in range(len(cell_data)):
            cell_arr = cell_data[i].split("(")
            cell_arr[1] = cell_arr[1].replace(")", "")
            if "," in cell_arr[1]:
                cell_0_str = cell_arr[0]
                cell_arr = cell_arr[1].split(",")
                cell_arr.insert(0, cell_0_str)
            for j in range(len(days_text_info_arr)):
                if day_str not in days_text_info_arr[j][5]:
                    continue
                if days_text_info_arr[j][3] == period_time:
                    if tab_id <= 1:
                        if CC_dictionary[days_text_info_arr[j][0]] == cell_arr[0] and \
                            BB_dictionary[days_text_info_arr[j][2]] == cell_arr[1]:
                            clickedInfo.append(days_text_info_arr[j])
                    else:
                        if CC_dictionary[days_text_info_arr[j][0]] == cell_arr[1] and \
                            BB_dictionary[days_text_info_arr[j][2]] == cell_arr[2]:
                            clickedInfo.append(days_text_info_arr[j])
    ...
    def setDate(self, idx):
        global season_tabarray
        season_tabarray = []
        now_season_id = AA_master_list[idx][0] + str(AA_master_list[idx][1])
        for i in range(len(df_time_period_list)):
            if df_time_period_list[i][0] == now_season_id:
                season_tabarray.append(df_time_period_list[i])
        self.retranslateUi()
    ...
    def retranslateUi(self):
        ...
        for i in range(len(clickedInfo)):
            ...
            for j in range(len(dialog_header)):
                ...
                for k in range(len(df_student_lesson_list)):
                    if clickedInfo[i][7] == df_student_lesson_list[k][2]:
                        student_name = EE_dictionary[df_student_lesson_list[k][1]]
                        break
                if j == 0:
                    item.setText(str(clickedInfo[i][3]) + "교시")
                elif j == 1:
                    ...
                    count = 0
                    for k in range(len(df_student_lesson_list)):
                        if student_name == EE_dictionary[df_student_lesson_list[k][1]]:
                            if clickedInfo[0][7] not in df_student_lesson_list[k][2]:
                                continue
                            count += 1
                    item.setText(str(count))
